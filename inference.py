...
def iou(det_a, det_b):
    ...
    distance_d = (center_a[0] - center_b[0]) * (center_a[0] - center_b[0]) + (center_a[1] - center_b[1]) * (center_a[1] - center_b[1])
    distance_c = (left_up[0] - right_down[0]) * (left_up[0] - right_down[0]) + (left_up[1] - right_down[1]) * (left_up[1] - right_down[1])
...
if __name__ == "__main__":
    ...
    for bbox in bboxes[0]:
        ...
        draw_box(src_img, bbox, color=color_map[bbox.class_id], text="{}-{:.2f}".format(name, bbox.obj_prob*bbox.class_prob))
